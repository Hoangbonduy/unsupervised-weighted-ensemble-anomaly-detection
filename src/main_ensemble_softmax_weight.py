...
# --- PHẦN 1: CÁC HÀM TIỆN ÍCH VÀ TÍNH TRỌNG SỐ MỚI ---

def calculate_softmax_weights(scores: dict, temperature: float = 5.0) -> dict:
    """
    Tính trọng số bằng hàm Softmax để làm nổi bật các mô hình tốt nhất.
    Nhiệt độ (temperature) càng cao, sự khác biệt về trọng số càng lớn.
    """
    ...
    # Áp dụng temperature và tính exponent
    exp_scores = np.exp(score_values * temperature)
    
    # Chuẩn hóa để tổng bằng 1
    sum_exp_scores = np.sum(exp_scores)
    if sum_exp_scores == 0: # Tránh chia cho 0
...
def visualize_ensemble_result(series, true_labels, predicted_labels, test_case_id, output_dir):
    ...
    # Thay đổi tiêu đề và tên file
    plt.title(f'Weighted Ensemble vs. Ground Truth for Test Case: {test_case_id}', fontsize=16)
    ...
    file_name = f'ensemble_result_test_{test_case_id}.png'
...
LABEL_DIR = 'labels'
OUTPUT_DIR = 'ensemble_results_softmax'

def main():
    print("Bắt đầu quy trình Weighted Ensemble (Softmax Weighting)...")
    ...
    # Sử dụng enumerate để đánh số thứ tự Test Case
    for test_case_num, place_id in enumerate(tqdm(all_place_ids, desc="Processing Test Cases"), 1):
        print(f"\n===== Đang xử lý Test Case: {test_case_num} (PlaceID: {place_id}) =====")
        original_series = data_full[data_full['placeId'] == place_id].sort_values('date')['view'].to_numpy().astype(float)
        n = len(original_series)
        if n < 100:
            print(f"Bỏ qua Test Case {test_case_num} do chuỗi quá ngắn.")
            continue

        # --- BƯỚC 1: TÌM TRỌNG SỐ CHO TEST CASE HIỆN TẠI ---
        ...
        # SỬ DỤNG SOFTMAX ĐỂ TÍNH TRỌNG SỐ
        weights = calculate_softmax_weights(avg_vus_scores, temperature=5.0)
        
        print("  Trọng số Softmax đã tính cho Test Case này:")
        for model, weight in sorted(weights.items(), key=lambda item: item[1], reverse=True): 
            print(f"    - {model:<10}: {weight:.4f} (Avg VUS-ROC: {avg_vus_scores.get(model, 0):.4f})")
        ...
        final_metrics_summary.append({
            'TestCase': test_case_num, 'PlaceID': place_id, 
            'CompF1': comp_f1, 'AffF1': aff_f1, 'TD': td
        })

        visualize_ensemble_result(original_series, true_labels, predicted_labels, test_case_num, OUTPUT_DIR)

    # --- BƯỚC 3: BÁO CÁO KẾT QUẢ TỔNG HỢP ---
    if final_metrics_summary:
        results_df = pd.DataFrame(final_metrics_summary)
        print("\n" + "="*80)
        print("BƯỚC 3: KẾT QUẢ CUỐI CÙNG TRÊN TẤT CẢ CÁC TEST CASE")
        print("="*80)
        print(results_df.round(3).to_string(index=False))
        print("-" * 80)
        print("Trung bình toàn cục:")
        print(results_df.drop(['TestCase', 'PlaceID'], axis=1).mean())
        print("="*80)
        
        output_file = os.path.join(OUTPUT_DIR, 'final_evaluation_metrics_all_tests.csv')
